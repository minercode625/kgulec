...
## `Overfull \vbox` 수치로 위험도 판정하기 (pt별 실제 증상)

컴파일은 성공하므로 경고를 흘려보내기 쉽다. 실측하면 수치가 증상과 거의 1:1로 대응한다.

| 넘침 | 실제로 벌어지는 일 |
|---|---|
| **30pt 이상** | 마지막 박스의 **본문이 통째로 사라진다.** 제목 띠만 남아 언뜻 정상으로 보인다 |
| **10~15pt** | 본문은 남지만 **박스 아래 테두리가 없어지고** 옆선이 footline으로 흘러든다 |
| 4~9pt | 테두리 살아 있고 간격만 0 — 무해 |

**판정 규칙: 10pt를 넘으면 무조건 실물을 확인한다.**

**검사 해상도가 결정적이다.** 70dpi PNG로는 「테두리 소실」을 못 본다 --- 사라진 테두리와 footline 선이 붙어 구분이 안 된다. **150~160dpi로 렌더**하고, 박스 아래 테두리와 footer 사이 간격을 픽셀로 재야 잡힌다. "보기엔 괜찮다"는 근거가 못 된다.

**고치는 법** 박스를 통으로 지우기보다 **줄 수를 줄인다**: 두 줄 설명을 한 줄로, `\\`로 나눈 항목을 `\quad`로 한 줄에 합치기, verbatim의 빈 줄 제거. 30pt짜리도 3~4줄만 줄이면 대개 해소된다.
