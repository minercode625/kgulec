...
**gotcha를 추가할 때는 매번 다음 3가지를 반드시 수행한다**(gotcha는 생성·수정마다 *전부 로드*되므로 비대해지면 매번 컨텍스트 비용이 된다):

1. **핵심만(용량 다이어트)**: 산문·중복 코드 예제를 걷어내고 `**증상**/**원인**/**해결**` + 위험/안전 최소 코드 1쌍만 남긴다. 시그니처(에러 문구)·임계값·해결 규칙은 보존.
2. **중복 체크**: 새 기록이 기존 항목(번들 + 사용자)과 같은 원인이면 **새 파일을 만들지 말고 기존 파일을 보강**한다.
3. **비슷한 내용 합치기**: 같은 메커니즘·도메인(예: 이미지 오버플로, tcolorbox 키-값 파싱, TikZ, 텍스트 특수문자)은 한 파일에 `##` 소제목으로 묶는다. 사용자 폴더에 유사 파일이 쌓였으면 합쳐서 정리한다.
