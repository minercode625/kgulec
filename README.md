...
> **권장 모델**: 자료 조사·구조 설계·LaTeX 생성이 복잡해 **Opus 이상(Claude Opus 4.x 등) 모델에서 가장 잘 동작한다.** 더 작은 모델은 컴파일 오류·레이아웃 품질이 떨어질 수 있다.
...
색 테마 + 공통 설정(교수명·소속·**주요 도메인(분야)**·기본 언어·기본 강의시간)을 한 번에 정해 저장한다. 이후 모든 생성물에 자동 적용되므로 매번 다시 묻지 않는다.

### 1. 강의자료 만들기

강의자료를 둘 **작업 폴더**(예: `week03/`)에서 Claude Code를 열고:

```
/kgulec:new
```

또는 자연어로 `"경사하강법 3시간 강의자료 만들어줘"`처럼 말해도 자동 트리거된다. 진행 흐름:

1. **자료 폴더 안내** — 작업 폴더에 `figs/`·`ref/`를 자동 생성하고 자료를 넣을지 묻는다(아래 참고).
2. **자료 조사** — 분야 기준으로 웹·`ref/`를 조사한다.
3. **Outline 승인** — 섹션 구성·슬라이드 수를 제시하고 승인받는다.
4. **생성·컴파일·품질검사** — `main.tex` 생성 → `pdflatex` 컴파일 → 오버플로·섹션 표지 검사·수정.

결과물: 작업 폴더에 `main.tex` + `main.pdf`.

### 2. `./figs` 와 `./ref` 폴더

모드 A 진입 시 작업 폴더에 두 폴더가 자동으로 생긴다. **선택 사항**이며, 비워 두고 "자료 없음 — 웹검색만으로 진행"을 골라도 된다.

| 폴더 | 무엇을 넣나 | 어떻게 쓰이나 |
|------|------------|---------------|
| **`./figs/`** | 슬라이드에 넣을 **이미지**(`.png` 등) | 현재 강의와 관련된 그림을 `\includegraphics`로 삽입. 적합한 그림이 없으면 `% TODO [IMAGE]` 주석 + 자리만 확보(나중에 파일 넣고 주석 해제) |
| **`./ref/`** | 강의자료 생성에 참고할 **문헌 자료(PDF)** | **1차 출처**로 삼아 범위·용어·표기·예시를 일관되게 함. 웹 검색은 보완용. 큰 PDF는 자동 요약 |

자료를 넣을 거면 폴더에 파일을 옮긴 뒤 "넣었어요"를 고르면, 다시 스캔해 반영한다.
...
또는 `"3번 슬라이드 예시 더 추가해줘"`, `"이 수식 고쳐줘"`처럼. 기존 `main.tex`를 수정·삭제·순서변경·교체·보강한다. 같은 `figs/`·`ref/` 규칙이 적용된다.
...
- 저장: 교수명·소속·**주요 도메인(분야)**·기본 언어·기본 강의시간·색 테마(이름 + 해석된 4색).
