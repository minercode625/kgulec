...
**전공 무관** — 슬라이드 생성 골격(템플릿·디자인·분량·오버플로 검사)은 분야에 종속되지 않는다. `/kgulec:setup`에서 지정한 **주요 도메인(분야)**에 맞춰 후보 토픽·예시가 생성되므로 CS/공학뿐 아니라 인문·사회·자연과학 등 어떤 전공 강의에도 쓸 수 있다. (단 실습코드 모드 `/kgulec:code`는 Python/HTML 코드 데모라 CS·공학 강의에 가장 잘 맞는다.)
